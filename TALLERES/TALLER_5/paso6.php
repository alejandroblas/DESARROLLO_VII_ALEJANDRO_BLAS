...
function crecimientoVentas($ventas) {
    ...
    foreach ($ventas as $producto => $valores) {
        // Calcula el crecimiento como el porcentaje del último mes respecto al primero
        $primerMes = $valores[0];
        $ultimoMes = end($valores);
        $crecimiento = (($ultimoMes - $primerMes) / $primerMes) * 100;
        $crecimientos[$producto] = $crecimiento;
    }
    
    return $crecimientos;
}

// 9. Calcular y mostrar el crecimiento de ventas por región y producto
echo "<br>Crecimiento de ventas por región y producto:<br>";
$mayorCrecimiento = ['producto' => '', 'region' => '', 'crecimiento' => -INF];

foreach ($ventas as $region => $productos) {
    echo "$region:<br>";
    $crecimientosPorRegion = crecimientoVentas($productos);
    
    foreach ($crecimientosPorRegion as $producto => $crecimiento) {
        echo "  $producto: " . number_format($crecimiento, 2) . "%<br>";

        // Identificar el mayor crecimiento
        if ($crecimiento > $mayorCrecimiento['crecimiento']) {
            $mayorCrecimiento['crecimiento'] = $crecimiento;
            $mayorCrecimiento['producto'] = $producto;
            $mayorCrecimiento['region'] = $region;
        }
    }
...
echo "Producto con mayor crecimiento: " . $mayorCrecimiento['producto'] . " en " . $mayorCrecimiento['region'] . " (" . number_format($mayorCrecimiento['crecimiento'], 2) . "%)<br>";
?>